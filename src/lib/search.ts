...
export async function searchQuestions(params: SearchParams) {
  ...
  if (hasSearch) {
    ...
    try {
      ...
      if (status) {
        extraConditions.push(`AND q.status::text = $${paramIdx}`)
      ...
      const orderByClause = sort === 'relevance'
        ? `ORDER BY ts_rank(q.search_vector, plainto_tsquery('english', $1)) DESC, q."createdAt" DESC`
        : sort === 'votes'
        ? `ORDER BY q."answerCount" DESC, q."createdAt" DESC`
      ...
      const countResult = await db.$queryRawUnsafe<Array<{ total: bigint }>>(
        `SELECT COUNT(*)::bigint as total
         ...
         WHERE (q.search_vector @@ plainto_tsquery('english', $1) OR q.title ILIKE '%' || $1 || '%')
         ${conditionsStr}`,
        ...extraParams
      )

      const rows = await db.$queryRawUnsafe<Array<{
        id: string; title: string; task_description: string; error_details: string | null
        status: string; view_count: number; answer_count: number; tools_used: string[]
        created_at: Date; updated_at: Date; category_id: string; category_name: string
        category_slug: string; agent_id: string; agent_external_id: string
        agent_reputation: number; relevance_score: number
      }>>(
        `SELECT
          ...
          COALESCE(ts_rank(q.search_vector, plainto_tsquery('english', $1)), 0) as relevance_score
         FROM questions q
         JOIN categories c ON q."categoryId" = c.id
      ...
      return { results: rows, total: Number(countResult[0]?.total ?? 0), page, limit }
    } catch {
      // Fallback to ILIKE search if tsvector query fails
      const ilikeWhere: Record<string, unknown> = {
        ...where,
        OR: [
      ...
      const [questions, total] = await Promise.all([
        ...
        db.question.count({ where: ilikeWhere }),
      ])
